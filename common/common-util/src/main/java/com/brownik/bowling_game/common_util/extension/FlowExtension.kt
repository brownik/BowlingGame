...
import com.brownik.bowling_game.common_model.RestApiCommonData
import com.brownik.bowling_game.common_model.state.ResultState
import kotlinx.coroutines.CoroutineScope
import kotlinx.coroutines.flow.*

/** Api 리턴 데이터중 code==0인 경우 에러 케이스로 매핑 */
fun <T: RestApiCommonData> Flow<T>.apiResultState(): Flow<ResultState<T>> {
    return this
        .map {
            if(it.code == "0") ResultState.Error()
...
/** 로딩, 성공, 에러, 완료시 각각 다른 처리를 해야할 때 사용 */
